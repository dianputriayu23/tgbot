...
async def format_schedule(lessons):
    if not lessons:
        return "Пар нет, на всякий случай проверьте пары на сайте pkeu.ru"
    
    date = lessons[0][3]
    day_of_week_ru = lessons[0][2].capitalize()
    header = f"<b>Расписание на {day_of_week_ru} ({date})</b>\n\n"
    ...
    for lesson in lessons:
        ...
        teacher = lesson[8]
        cabinet = lesson[9]
        resource_link = lesson[10] if len(lesson) > 10 else ""
        
        # Get emoji for lesson number
        emoji = EMOJI_NUMBERS.get(lesson_num, "🔹")
        
        schedule_text += f"{emoji} <b>{subject}</b>\n"
        schedule_text += f"Время: {time_start}-{time_end}\n"
        if teacher:
            schedule_text += f"Преподаватель: {teacher}\n"
        if cabinet:
            schedule_text += f"Кабинет: {cabinet}\n"
        if resource_link:
            schedule_text += f"Ресурс: {resource_link}\n"
        schedule_text += "\n"

...
async def cmd_settings(message: Message, db: Database):
    ...
    notify_l = "✅ Включено" if user[6] else "❌ Выключено"; notify_c = "✅ Включено" if user[7] else "❌ Выключено"
    await message.answer(f"<b>⚙️ Ваш профиль:</b>\n<b>ID:</b> <code>{user[0]}</code>\n<b>Курс:</b> {user[4]}\n<b>Группа:</b> {user[5]}\n\n<b>Уведомления о парах:</b> {notify_l}\n<b>Уведомления об изменениях:</b> {notify_c}", reply_markup=get_settings_kb(user))
...
async def cq_settings_actions(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    if "toggle" in action:
        new_user = await db.get_user(user[0]); notify_l = "✅ Включено" if new_user[6] else "❌ Выключено"; notify_c = "✅ Включено" if new_user[7] else "❌ Выключено"
        await callback.message.edit_text(f"<b>⚙️ Ваш профиль:</b>\n<b>ID:</b> <code>{new_user[0]}</code>\n<b>Курс:</b> {new_user[4]}\n<b>Группа:</b> {new_user[5]}\n\n<b>Уведомления о парах:</b> {notify_l}\n<b>Уведомления об изменениях:</b> {notify_c}", reply_markup=get_settings_kb(new_user))

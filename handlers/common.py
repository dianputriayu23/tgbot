...
async def cmd_start(message: Message, state: FSMContext, db: Database):
    ...
    # Generate static_id if not exists
    if user and not user[8]:  # static_id is at index 8
        await db.generate_static_id(message.from_user.id)
        user = await db.get_user(message.from_user.id)
...
async def cmd_settings(message: Message, db: Database):
    ...
    static_id = user[8] if user[8] else "Не назначен"
    notify_schedule = "✅ Включено" if user[6] else "❌ Выключено"
    notify_changes = "✅ Включено" if user[7] else "❌ Выключено"
    
    await message.answer(
        f"<b>⚙️ Ваш профиль:</b>\n"
        f"<b>Static ID:</b> <code>{static_id}</code>\n"
        f"<b>Курс:</b> {user[4]}\n"
        f"<b>Группа:</b> {user[5]}\n\n"
        f"<b>Уведомления о расписании:</b> {notify_schedule}\n"
        f"<b>Уведомления об изменениях:</b> {notify_changes}",
...
async def cq_settings_actions(callback: CallbackQuery, state: FSMContext, db: Database):
    ...
    if action == "change_group":
        await callback.message.edit_text("На какой базе ты учишься?", reply_markup=get_education_form_kb())
        await state.set_state(Registration.choosing_education_form)
    elif action == "toggle_schedule":
        await db.update_user_notifications(user[0], notifications_schedule=not user[6])
    elif action == "toggle_changes":
        await db.update_user_notifications(user[0], notifications_changes=not user[7])
    
    if "toggle" in action:
        new_user = await db.get_user(user[0])
        static_id = new_user[8] if new_user[8] else "Не назначен"
        notify_schedule = "✅ Включено" if new_user[6] else "❌ Выключено"
        notify_changes = "✅ Включено" if new_user[7] else "❌ Выключено"
        
        await callback.message.edit_text(
            f"<b>⚙️ Ваш профиль:</b>\n"
            f"<b>Static ID:</b> <code>{static_id}</code>\n"
            f"<b>Курс:</b> {new_user[4]}\n"
            f"<b>Группа:</b> {new_user[5]}\n\n"
            f"<b>Уведомления о расписании:</b> {notify_schedule}\n"
            f"<b>Уведомления об изменениях:</b> {notify_changes}",
            reply_markup=get_settings_kb(new_user)
        )

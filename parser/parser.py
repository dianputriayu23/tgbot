...
async def run_initial_parsing(db: Database):
    ...
    if file_path and file_hash:
        ...
        if not existing_hash:
            ...
            old_schedule = await db.execute("SELECT group_name, day_of_week, lesson_number, subject FROM schedule", fetch='all')
            await parse_schedule(file_path, file_hash, db)
            
            # Notify users about new schedule
            if _bot_instance:
                await notify_schedule_changes(db, _bot_instance, old_schedule)
            
            logging.info("[SCHEDULER] Update complete.")
        else:
            logging.info("[SCHEDULER] No new schedule files found.")

async def notify_schedule_changes(db: Database, bot, old_schedule):
    """Notify users about schedule changes"""
    logging.info("[NOTIFICATIONS] Checking for schedule changes...")
    
    # Get new schedule
    new_schedule = await db.execute("SELECT group_name, day_of_week, lesson_number, subject FROM schedule", fetch='all')
    
    # Convert to sets for comparison
    old_set = set(old_schedule) if old_schedule else set()
    new_set = set(new_schedule) if new_schedule else set()
    
    if old_set != new_set:
        # Get users who want change notifications
        users = await db.get_users_with_notifications('changes')
        
        for user_id, group_name in users:
            try:
                message = "🔔 <b>Внимание! Расписание изменилось!</b>\n\nПроверьте актуальное расписание с помощью кнопок меню."
                await bot.send_message(user_id, message)
            except Exception as e:
                logging.error(f"Error notifying user {user_id} about changes: {e}")
        
        logging.info(f"[NOTIFICATIONS] Notified {len(users)} users about schedule changes.")
                ...
                for group, cols in current_groups.items():
                    ...
                    if subject:
                        ...
                        all_lessons.append({
                            ...
                            "subject": subject, "teacher": teacher, "cabinet": cabinet, "file_hash": file_hash
                        })

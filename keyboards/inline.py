...
def get_settings_kb(user_data):
    builder = InlineKeyboardBuilder()
    notify_lessons_text = "✅ Уведомления о парах" if user_data[6] else "❌ Уведомления о парах"
    notify_changes_text = "✅ Уведомления об изменениях" if user_data[7] else "❌ Уведомления об изменениях"
    notify_new_text = "✅ Уведомления о новом расписании" if len(user_data) > 8 and user_data[8] else "❌ Уведомления о новом расписании"
    
    builder.button(text="Сменить группу", callback_data="settings:change_group")
    builder.button(text=notify_lessons_text, callback_data="settings:toggle_lessons")
    builder.button(text=notify_changes_text, callback_data="settings:toggle_changes")
    builder.button(text=notify_new_text, callback_data="settings:toggle_new_schedule")
    builder.adjust(1)
    return builder.as_markup()